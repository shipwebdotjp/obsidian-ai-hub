...
class HitlContext:
    ...
    def register_next_questions(
        ...
        handler_name = run["handler"] if run else "unknown"
        register_run_and_questions(
            run_id=self.run_id,
                ...
                with conn:
                    ...
                    if run:
                        if result.status == "completed":
                            ...
                            # If the handler called register_next_questions, the run's status
                            # and active set have been updated, and lease released.
                            # We reload the run to avoid overwriting those changes.
                            refreshed_run = get_run(run_id, conn)
                            if refreshed_run:
                                run = refreshed_run

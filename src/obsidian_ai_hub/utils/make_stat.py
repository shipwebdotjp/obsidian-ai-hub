...
def should_skip_session(md: dict) -> bool:
    ...
    keys = ["topics", "intent", "sentiment", "complexity", "value", "confidence", "keywords", "summary"]
...
def top_k(stats: dict[str, Stat], k: int) -> list[str]:
    ...
    items.sort(key=lambda kv: (kv[1].count, kv[1].conf_sum, kv[1].latest_ts), reverse=True)
    return [label for label, _ in items[:k]]

def pick_majority(stats: dict[str, Stat], default: str, *, is_complexity: bool = False) -> str:
    if not stats:
        return default
    items = list(stats.items())
    # 多数決 + タイブレーク
    ...
    # 先頭と同率グループを抽出
    best = items[0][1]
    tied = [kv for kv in items if (kv[1].count, kv[1].conf_sum, kv[1].latest_ts) == (best.count, best.conf_sum, best.latest_ts)]
...
def aggregate_daily(ai_logs_metadata_only: dict) -> dict:
    ...
    kw_items.sort(key=lambda x: (x[1][1].count, x[1][1].conf_sum, x[1][1].latest_ts), reverse=True)

...
## 個人開発ではブラウザ E2E を追加・更新しない

| 項目 | 内容 |
|------|------|
| 決定日 | 2026-08-20 |
| カテゴリ | テスト戦略・フロントエンド |
| 決定内容 | ブラウザ E2E は今後追加・更新せず、通常の完了条件や定常実行にも含めない。画面の確認は開発者が手動で行い、必要な自動検証は単体テストまたは結合テストで行う。 |

### 結論に至った経緯

本プロジェクトは個人利用を前提とし、画面の不具合は利用中に見つけて修正できる。ブラウザ E2E は画面構成やセレクタの変更に追随する保守負債になりやすく、その費用は継続的な自動回帰検知の利益を上回ると判断した。

### 適用範囲

- `tests/e2e/`・`make test-e2e`・`frontend-e2e.yml` は削除済み（コミット `dd42e4c`）。ブラウザ E2E は
  追加・更新・通常実行しない。
- フロントエンド変更後は、影響した画面を手動で確認する。
- 状態遷移、API契約、データ整合性などの自動検証は、必要に応じてフロントエンドの単体テストまたはバックエンドの結合テストで行う。フロントエンド境界でのモックを許容する。

### トレードオフ

実ブラウザでのみ起きる回帰の自動検知は行わない。その代わり、実装の変更に追随する脆いテストとブラウザ実行の負担を増やさず、個人利用に見合う手動確認と軽量な自動テストに集中する。

### 未決事項（テスト被覆）

- 破壊的操作（People のマージ/削除、Scheduler Job の PUT）はブラウザ E2E に依存しない方針だが、
  Vitest + Python TestClient で十分かを必要時に判断する。
- FastAPI の SPA フォールバック契約（任意パスで `index.html`）は、単体/結合テストで代表ケースを持つ。

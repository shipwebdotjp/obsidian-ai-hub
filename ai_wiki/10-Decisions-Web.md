...
## Safari の検索結果リストの隙間（inline-block ボタンのラインボックス問題）

| 項目 | 内容 |
|------|------|
| 決定日 | 2026-08-26 |
| カテゴリ | フロントエンド・UI（クロスブラウザ） |
| 決定内容 | エージェント会話履歴の横断検索結果で、Safari（Mac/iOS）のみ結果アイテム同士に余分な隙間が入る。原因は `<button>` を `inline-block` のままリスト要素にしていること。`display:block` にし、テキストの2行クリップは `line-clamp`（`-webkit-box`）ではなく `max-height` 方式を使う。 |

### 事件の経緯

`/agents` の会話履歴検索（`sessionSearchResults`）は、検索結果ごとに `<button>` を直接リスト要素として描画する。Chrome では結果が隙間なく並ぶ一方、Safari では結果同士に隙間が入り、その量が検索ワードやスニペット内容に応じて変わる不安定な挙動だった。

- 「イエス」検索: 3件すべての間に隙間。
- 「熱中症」検索: 1–2件目は隙間なし、2・3件目のスニペット**後**に余白。

### 根本原因

当初 `line-clamp-2`（`display:-webkit-box` + `-webkit-line-clamp`）の Safari ファントム行（クリップされる要素だけ末尾に空行が乗る）を疑い、`max-h-8 overflow-hidden` に置換した。しかし DOM には新クラスが正しく反映されているのに隙間は残ったため、原因はコンテンツ高ではなく**ボタンのレイアウト**だった。

`<button>` は UA スタイルシートで `inline-block` が既定。Safari は inline-block 要素を**ベースライン整列のラインボックス**で並べるため、各ボタンの下にラインボックス分の余白が入る。その余白量は各ボタンの内容（スニペットの行数→ベースライン位置）に依存するため、内容依存で不安定な隙間になる。Chrome は inline-block のラインボックス計算が異なり、また同じペインの会話一覧・エージェント一覧は `<div>` 要素のため、検索結果だけが顕在化した。

### 対処

1. 検索結果ボタン（`frontend/src/features/agents/AgentsPage.tsx`）に `block` を付与し `inline-block` を廃止。ブロック要素同士の `space-y-1` は均等マージンのみで、ラインボックス由来の余白は発生しない。
2. テキストの2行クリップは `line-clamp-2` ではなく `max-h-8 overflow-hidden leading-4` に変更（`text-xs` の line-height 1rem ×2 = 2rem）。スニペットの省略記号 `…` はバックエンド（`agents/store.py::_make_search_snippet`）が `" ".join(content.split())` で空白正規化後に付与済みのため、CSS 側の ellipsis は不要。

### 検証

- `npx vite build` クリーン。DOM 上のクラス反映を確認。
- Safari 実機（Mac/iOS）で検索結果の隙間が解消することを確認。

### トレードオフ

- `max-height` 方式はハードクリップのため、2行を超える長文は末尾が省略記号なしで切れるが、バックエンドが `…` を付与済みのため見た目は損なわれない。
- 今後 `line-clamp` を使う箇所は、Safari の `-webkit-box` ファントム行と inline-block のラインボックス問題の両方を踏まえること。
...
|------|------|
| 決定日 | 2026-08-29 |
| カテゴリ | Web API・リサーチ UI |
| 決定内容 | リサーチテーマの重複情報はIDだけでなく重複先テーマ名を返し、一覧・詳細からそのテーマの詳細を開けるようにする。 |

### 設計と理由

- `ResearchTheme` API に後方互換の `duplicate_of_theme: {theme_id, theme} | null` を追加し、既存の `duplicate_of_theme_id` は維持する。重複先の解決は一覧クエリの自己結合と単体取得クエリで行い、一覧表示時のN+1取得を避ける。
- UI はテーマ名を主表示にして遷移可能な操作とする。IDは重複先レコードが存在しないデータ不整合時だけ補助情報として表示し、正常時に人がIDを解読する必要をなくす。
- 重複先が存在しない場合もテーマ一覧・詳細の取得自体は成功させる。参照は `null` とし、UIでは遷移操作を表示しない。

## Coding画面のPC左ペイン統合（2ペイン化）

| 項目 | 内容 |
|------|------|
| 決定日 | 2026-08-30 |
| カテゴリ | コーディング・Web UI |
| 決定内容 | CodingPage.tsx のPCレイアウトを、従来の2本の一覧列から幅 w-64 の左ペイン1本（上部：プロジェクト一覧、下部：セッション一覧の上下等分・独立スクロール）と右側会話領域の2ペイン構成へ統合する。 |

### 結論に至った経緯

AgentsPage と共通のデザインパターンを採用し、PC幅での情報密度と操作性を一貫させる。従来のセッションヘッダー内にあった「既定設定」ボタンおよび「+ 新規」ボタンは、下部のセッション一覧ヘッダーに維持する。

### 仕様と動作契約

1. **PC 2ペイン構造:**
   - 幅 `w-64` の単一左ペインへ統合。`flex-1 flex-col min-h-0` で上部（プロジェクト一覧）と下部（セッション一覧）に上下等分し、それぞれ独立したスクロール（`overflow-y-auto`）を持つ。
2. **非永続の開閉状態:**
   - 左ペイン上部に「サイドバーを畳む」ボタン（`ChevronLeft`, `aria-label="サイドバーを畳む"`）を配置。
   - 畳み込み時は、会話ヘッダー（セッション選択時）および空状態（セッション未選択時）から「サイドバーを展開」ボタン（`ChevronRight`, `aria-label="サイドバーを展開"`）で復帰可能。
   - 開閉状態 (`leftPaneCollapsed`) は React コンポーネント内の state でのみ保持し、ページ再表示時は常に展開された初期状態に戻す。
3. **モバイル挙動およびAPI・永続化の非変更:**
   - モバイル用のドロワー（`mobileDrawerOpen`）およびそのトリガー・選択・フォーカス管理挙動は変更せず維持する。
   - バックエンド API・型・SQLite 永続化層の変更は一切行わない。

...
## auto リサーチのプロジェクト自動ルーティング

| 項目 | 内容 |
|------|------|
| 決定日 | 2026-09-22 |
| カテゴリ | リサーチ・LLM連携・コードベース調査 |
| 決定内容 | `auto` モードのリサーチルーターに、有効な Git リポジトリを持つ登録プロジェクトの候補（整数ID・名称・目的・説明・キーワードのみ）を渡し、構造化 JSON 応答が単一候補へ高確信（`confidence >= 0.85`）を示した場合だけ実行モードを `project` に上書きする。`project` 確定時は Coding Agent 起動前に `research_themes.project_id` と `research_jobs.project_id` を同一トランザクションで保存する。 |

### 結論に至った経緯

テーマによっては、一般知識や Web ではなく、登録済みプロジェクトの実装・設計・履歴を読むのが最も適切な場合がある。既存の `project` モードは利用者が明示的に `--project-id` を指定する前提で、auto は常に internal / web / deep のいずれかに落ちていた。そこで auto の判断材料にプロジェクト候補を加え、関連が明確なときだけコードベース調査へ昇格させる。

### 安全境界と判断理由

- **候補の露出範囲:** ルーターへ渡すのは有効な Git リポジトリを持つプロジェクトの整数ID・名称・目的・説明・キーワードのみ。`project_path` やコード本文は渡さない。
- **受理条件:** `project_id` は候補一覧に含まれる整数IDのみを受理し、確信度が `0.85` 未満・未知ID・不正/旧形式/不明瞭な応答は、追加 HITL を出さず既存の internal / web / deep へフォールバックする。複数候補が有力、または関連が弱い場合も `project` にしない。
- **明示優先:** 対象は auto モードのみ。利用者が指定した `project` / internal / web / deep、および既に `project_id` を持つ auto はルーターを介さず利用者指定を優先する。
- **保存:** 自動選択が確定した場合、Coding Agent 起動前にテーマとジョブの `project_id` を同一トランザクションで更新する（`research.db.assign_project`）。失敗時も選択結果が残り、テーマとジョブのスコープが食い違わない。
- **失敗時の挙動:** Coding Agent の失敗は web / internal へ黙って代替せず、ジョブを `failed` にする。既存の一時 detached worktree による読み取り専用調査契約は維持し、成功時のみ Vault へ保存する。
- **公開インターフェース:** 公開 API・CLI の入力形式、明示的な `project` モード、DB スキーマは変更しない。ルーターのプロンプトと応答形式のみを構造化する。

### トレードオフ

- ルーター応答が JSON でない・壊れている場合は従来の単語応答（internal / web / deep）を解釈して継続し、それも不能なら internal に落ちる。判断を LLM に委ねるため、関連が弱いテーマが `project` になる可能性は残るが、高確信しきい値と allowlist で抑制する。
- 候補生成のたびに各プロジェクトで `git rev-parse` を実行するため、プロジェクト数に比例してルーティングがわずかに遅くなる。候補がない場合は LLM にプロジェクト判断を委ねない。

### 承認説明・ユーザーガイド

- HITL の承認説明に、auto が関連する登録済み Git プロジェクトを読み取り専用で調査する場合がある旨を明記する。別の確認画面は追加しない。
- ユーザーガイドのリサーチ機能に同じ挙動を記載する。

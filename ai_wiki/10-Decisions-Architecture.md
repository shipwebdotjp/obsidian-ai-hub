...
## AI エージェント単発送信 CLI (--agent-chat)

| 項目 | 内容 |
|------|------|
| 決定日 | 2026-09-05 |
| カテゴリ | AIエージェント・CLI・永続化 |
| 決定内容 | `python -m obsidian_ai_hub --agent-chat` を追加し、既存エージェントへ本文を1ターン送信して会話・実行記録（`agent_sessions`, `agent_messages`, `agent_runs`）を SQLite に永続化する。--resume-session 未指定時は新規会話を作成し、指定時はセッションの存在と agent_id の一致を検証した上で既存会話へ追記する。エージェントに設定済みの provider/model、パラメータ、ツール権限、HITL境界、初回会話のタイトル自動生成をそのまま適用する。 |

### 結論に至った経緯

Web UI だけでなく、シェルスクリプトや外部自動化から既存の AI エージェントを呼び出して会話を継続・記録したいニーズがあった。Web UI と CLI で異なる処理ロジックやデータモデルを作ると、権限設定やログ記録の不整合が生じるため、エージェント会話ターンの準備ロジック（`prepare_session_turn`）を `agents` サブパッケージ側に集約し、Web API (`stream_session_message`) と CLI (`main_agent_chat`) の双方から再利用する構成とした。

### 構造と出力契約

- **入力切り替え**: `--agent-prompt` が未指定かつ `sys.stdin.isatty()` が `False` の場合は標準入力全文を使用する。空白本文や TTY 端末での本文未指定はエラーメッセージを出力して非0終了する。
- **初版制限（テキスト本文のみ）**: 画像添付や対話型 REPL、CLI からのエージェント設定上書きは初版の対象外とし、既存エージェント設定に従う。
...
- **Agent 定期 Job 所有**: `register_recurring_job` は active YAML を全件読み、`agent_source`（登録元 ID と UTC 時刻）付きで `jobs/jobs.local.yml` へ原子的に書く。定期定義の正本は YAML のままとし、DB migration・新規 API は追加しない。`set_recurring_job_enabled` は `agent_source.agent_id` が trusted agent と一致する未編集ジョブだけを対象とし、欠落・破損 source は未所有として拒否する。破損 source は runner と一覧を止めない。人間の `/jobs` 更新が意味変更（ID/command/schedule/enabled）なら source を失効させ、同値 PUT・並び替え・client 供給 source は保持／無視する。Task Agent へは `register_recurring_job` のみ `plan_required` で公開し、`set_recurring_job_enabled` は `EXCLUDED_TOOL_IDS` で除外して停止操作を人間に限定する。操作契約は `docs/job/recurring-jobs.md`。

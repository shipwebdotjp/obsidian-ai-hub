...
4. **共通UIコンポーネント (`WaitingRunQuestionCard`):**
 ...
 9. **ask_user 回答履歴の会話内表示と checkpoint 正規化:**
    - セッション詳細 API（`get_session_detail`）において、親 Run の `hitl_run_id` 紐付けと HITL checkpoint 内の `qa_history`（v2）および単一 `answers`（v1）から `ask_user_answer_history` を動的に抽出・正規化して返却するアプローチを採用する。
    - 追加の DB テーブルやカラムといったスキーマ拡張を行わない理由:
      1) 既に HITL checkpoint (`qa_history`) に各回答ラウンドの質問ID・質問文・選択肢・ユーザー回答・自由入力テキストが構造化して保持されており、情報の二重保持や同期不整合リスクを防止できるため。
      2) セッション詳細ロード時に親ユーザーメッセージ ID（`user_message_id`）に紐付けて API 応答レベルで動的に正規化することで、過去ラウンドも含めた後方互換性と高速なレスポンスを両立できるため。
      3) 未回答・取消・失敗・期限切れ・非 ask_user の HITL Run はフィルタリングで除外され、確定した回答のみを正しく親メッセージ直後に時系列順で投影できるため。

## 会話内HITLの画面表示ラベル短縮と回答履歴カードの導入

| 項目 | 内容 |
|------|------|
| 決定日 | 2026-09-05 |
| カテゴリ | HITL・UI・会話内要件確認 |
| 決定内容 | `HitlPage` において、`display_type === "in_conversation_question"` の画面表示バッジを `"ASK"` に短縮し、保存済みの回答 (`status === "answered"`) は「回答済み要件確認」として読取り専用のカードで表示する。未回答フォームは未回答質問が存在する場合のみ操作可能とし、空の未回答フォームを表示しない。 |

### 結論に至った経緯と表示方針

1. **画面表示ラベルの短縮 ("ASK"):**
   - 内部識別子・API・永続化層の `display_type`（`in_conversation_question`）およびコード内の判定ロジックは変更せずそのまま維持する。
   - `HitlPage` の一覧バッジおよび詳細ヘッダーバッジのみ、表示テキストを `"ASK"` へ変換して描画し、視認性と省スペース化を向上させる。

2. **回答済み質問の読取り専用履歴カード表示:**
   - `status === "answered"` の質問は、質問文、選択肢から解決した回答ラベル、および `"other"` 選択時の自由入力本文（コメント）をエメラルド基調の「回答済み要件確認」カードとして表示する。
   - 選択肢が変更・削除等でラベル解決できない場合は保存済みの値を表示する。
   - Runのステータス（`ready_to_resume`、`completed`、`failed`、`cancelled`）に関わらず、回答済みの質問が存在すれば常にこのカードを表示する。

3. **未回答質問（`pending`）とフォーム制御:**
   - 回答済み（`answered`）と未回答（`pending`）の質問が混在する場合は、先に回答済み履歴カードを表示し、その下に `pending` の質問のみを対象とした `WaitingRunQuestionCard` を描画して操作可能にする。
   - 全質問が回答済み（`pending` なし）の場合は、`WaitingRunQuestionCard` および送信・取消ボタン、未回答用見出しを描画せず、空の未回答フォームを残さない。
   - Run全体の取消操作（「実行全体をキャンセル」ボタン）は、従来通り Run 自体がキャンセル可能なステータス（`pending_user` / `ready_to_resume`）である場合に限って上部に維持する。

## HITL の未実装・保留事項（2026-09-19）

判断済みだが意図的に実装していない、または判断を保留している事項。

- **LINE 受信イベントの処理 worker と Reply API は未実装**。Webhook 受信の署名検証・重複排除付き記録と、
  自動提案の push 通知までが実装範囲。LINE 上で回答を完結させるかは「通知専用＋Web UI への深いリンクを
  主導線」という現行決定のままで、将来必要になった場合の論点（回答の自動実行範囲、失敗時の再試行・
  通知・手動復旧）は未決。
- **Outbox / 通知再送 / 外部エージェント用の質問作成 API は未実装**。通知漏れが運用上問題になった段階で
  追加する。
- **回答履歴の改訂、質問単位の取消は未実装**。Run 単位の取消のみ提供する。期限切れの自動解決は実装済み。
- **確認待ち画面の角丸は `frontend/AGENTS.md` の配色・角丸規約から逸脱が残る**（選択状態の逸脱は解消）。
- HITL MVP 設計時の検討資料（質問キュー設計調査、MVP 設計書）は判断記録へ統合済みとして削除した。

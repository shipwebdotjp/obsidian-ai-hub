...
特定の人物に紐づく事実、好み、傾向、約束、および人生の重要転機に関する長期記憶（`scope = 'person'`）。日次サマリの人物メモから週次バッチ（`extract_person_memories`）で自動抽出され、1メモリを複数人物（`memory_people`）で共有できる。利用者自身の全般記憶（`scope = 'user'`）とはCopilot投影・重複統合において分離される。承認済み有効メモリのAIコンテキストへの露出は、`people_get` ツール実行時、および人物文脈を扱う purpose（日次・週次要約など `include_person = true` のポリシー）でのコンパイル時に、人物表示名つきの別セクションとして最小投影に限定される。

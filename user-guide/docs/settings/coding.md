...
- オーケストレーター（進行役）のプロバイダーとモデルは **会話設定 ⚙** ダイアログで会話ごとに変更できます。
- 未設定（「既定を使用」）の場合、そのセッションは `coding.orchestrator.provider` / `coding.orchestrator.model` の既定値を使用します。
- 変更は次回送信（次の run）から反映されます。実行中の run は開始時に固定した値を使い続けます。
- プロバイダーは対応するもの（`openai` / `gemini` / `ollama` / `local` / `opencode_go`）から選択し、モデルは自由入力です。

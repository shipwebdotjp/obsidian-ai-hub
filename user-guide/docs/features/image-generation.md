...
アシスタントの応答本文に `![説明](/api/v1/media/{media_id})` と書かれている場合、
その場に画像がインライン表示されます（認証付きで取得するため、Markdown を
そのままコピーしてもブラウザ単体では表示されません）。

- 本文に埋め込んだ画像と同じものは、ツール結果の画像カードには重複表示されません。
- 生成物以外の画像（外部 URL）は通常の画像として表示されます。`data:` などの
  安全でない形式は表示されず、代替テキストになります。

...
`/agents` を `session_id` なしで開いたとき（直接アクセス、リロード、SPA内のサイドバー遷移等）、同一ブラウザプロファイル内で最後に表示対象となった会話セッション（最終表示会話）を復元する。

- 保存先は `localStorage`（キー `obsidian-ai-hub:agents-last-viewed-session:v1`、値 `{"session_id": "...", "savedAt": "..."}`）とし、ブラウザ再起動後も維持する。
- 有効な deep link（`/agents?session_id=<id>`）は常に最優先し、保存値は `session_id` がない場合だけ使う。
- 会話が実際に選択・表示対象になった時点で保存する（一覧選択、新規作成、検索結果選択、有効 deep link、IDなし通常フォールバック、無効値からの先頭フォールバック）。詳細取得の競合で古い応答が後着した場合は保存しない。
- 保存IDは deep link と同程度に実在・所属を検証する。別agent所属でも対象agentを解決して開く。削除済み・アクセス不能・不正で復元できない場合は保存値を消去し、無効 deep link と整合する形で先頭会話へ無言フォールバックし、フォールバック先を新たに保存する。
- ストレージ例外（プライベートモード・容量・破損値）は無言で従来の先頭フォールバックに劣化する。URL同期は `replace: true` を維持し、戻る／進むでの会話切替は行わない。

受け入れ条件:

- 保存IDあり・`session_id` なしでその会話（別agent所属含む）が復元される。
- 有効 deep link が保存値より優先される。
- 無効な保存IDでは保存値が消去され、先頭会話へフォールバックしてそれが保存される。
- localStorage 例外時も先頭フォールバックで画面が利用できる。
- 一覧選択・新規作成・検索結果選択・deep link 選択で選択確定時に保存される。

恒久判断は [最終表示会話の復元](../../ai_wiki/10-Decisions-Web.md#agents-の最終表示会話の復元localstorage)を正とする。

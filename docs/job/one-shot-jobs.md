...
| --- | --- | --- | --- | --- | --- | --- |
| Agent登録 | Pydantic 入力（`command` 必須、`run_at` 任意）、trusted Agent context | `job_id`（uuid4 hex） | `queued` job（`run_at_utc`、`agent_id`/`session_id`/`run_id` 付き） | runner の claim | 不正 command・不正日時は保存しない（`ValueError`） | SQLite 書込み |
...
- UI からの手動登録は提供しない（一覧・詳細・未開始取消のみ）。

## API と画面

- `GET /api/v1/scheduler-jobs/one-shot-jobs?limit=100&offset=0` — 未完了（`queued`/`running`、`run_at` 昇順）を優先し、終端履歴（`finished_at` 降順）を続けて返す。
...
- `/jobs` の「ワンショット実行ジョブ」欄に予定時刻・状態・登録元・command・終了コードを一覧化し、詳細表示と未開始分の取消を提供する。いずれも Bearer 認証。

...
- **Access Control:** The configuration APIs (`GET`/`PUT` `/api/v1/task-config`) are restricted to loopback clients and, when `OBSIDIAN_AI_HUB_ALLOW_TAILNET_TASKS=1` is set, to Tailscale tailnet clients (`100.64.0.0/10`) that present a valid `OBSIDIAN_AI_HUB_API_TOKEN` Bearer token. Any other request (LAN or public network, including Tailscale Funnel) is rejected with `403 Forbidden` to prevent unauthorized execution of local commands. Talnet access is disabled by default (fail-closed).

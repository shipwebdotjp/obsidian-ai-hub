...
def test_resume_with_gapped_indices_continues_after_max():
    ...
    outcome = run_directional_plan(task["task_id"], plan, executor, generator)
    assert outcome.kind == "completed"
    # max(action_index)+1 == 4: the resumed action must not collide with 0/3.
...
def test_deviation_rebuild_preserves_completed_targets():
    ...
    task = store.create_task("delegate job")
    plan = store.create_plan(
        task["task_id"],
        {
            ...
            "strategy": "",
            "capabilities": [
                {"capability_key": "specialist_agent", "intent": "delegate"}
            ],
    ...
    plans = store.list_plans(task["task_id"])
    assert len(plans) == 2
    revised_steps = plans[1]["plan"]["steps"]
    assert revised_steps[0]["target"] == {"agent_id": "agent_1"}
    assert revised_steps[0]["inputs"] == {"task": "first"}
...
def test_in_scope_agent_target_executes():
    task = store.create_task("委譲ジョブ")
    plan = _directional_plan_with_scope(
        task["task_id"], ("specialist_agent",), allowed_agents=["agent_1"]
    )
    ...
    generator = _scripted(
        [
            ...
            {"action": "finish", "summary": "delegated", "reason": "ok"},
        ]
    )
    outcome = run_directional_plan(task["task_id"], plan, executor, generator)
    assert outcome.kind == "completed"
...
def test_prompt_history_is_redacted():
    ...
    history = [
        {
            "action_index": 0,
            "capability_key": "vault_search",
            "inputs": {"query": "leak-topsecret-1"},
            "observation": "result topsecret-1 here",
        }
    ]
    prompt = orchestrator_module.build_orchestrator_prompt(
        {"prompt_text": "job"},
    ...
    assert "topsecret-1" in prompt  # baseline: no secrets configured
    redaction_configured = redaction.configured_secret_values
    try:
        redaction.configured_secret_values = lambda: ("topsecret-1",)
        prompt2 = orchestrator_module.build_orchestrator_prompt(
            {"prompt_text": "job"},
            orchestrator_module.DirectionalPlan(
                purpose="p",
                capabilities=[{"capability_key": "vault_search", "intent": "i"}],
        ...
        redaction.configured_secret_values = redaction_configured
    assert "topsecret-1" not in prompt2
    assert "[REDACTED]" in prompt2

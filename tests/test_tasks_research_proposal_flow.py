"""Integration scenario: 6-Action research proposal Task.

The plan allows four read capabilities, the proposal capability, and finish.
The Runtime Orchestrator must run the four reads, then propose exactly once,
then finish. Re-running after a crash between the proposal side effect and
its completion event must not register a second HITL candidate because
registration is idempotent per Task ID.
"""
...
def test_six_action_research_proposal_registers_one_candidate(monkeypatch):
    ...
    generator = _scripted(
        ...
        + [
            ...
            {"action": "finish", "summary": "1件提案した", "reason": "done"},
        ]
    )
    ...
    assert outcome.result_summary == "1件提案した"

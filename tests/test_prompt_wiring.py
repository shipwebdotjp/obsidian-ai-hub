...
def test_prompt_externalization_wiring(mock_dependencies):
    ...
    # scan_line_inbox
    with (
        patch("obsidian_ai_hub.scan_line_inbox.accessibility") as mock_acc,
        ...
        mock_acc.get_line_window.return_value = {
            "window_id": 123,
            "window_title": "LINE",
        }
        ...
        assert (
            mock_prompt.render_prompt.call_args[0][0]
    ...
    with (
        patch("obsidian_ai_hub.summerize_week.llm_client") as mock_llm,
    ...
    with (
        patch("obsidian_ai_hub.research.runner.prompt") as mock_prompt,

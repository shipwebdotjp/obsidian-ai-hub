...
def test_run_dedup_llm_duplicate():
    ...
    llm_response = '{"decision": "duplicate", "target_theme_id": "' + t1["theme_id"] + '", "related_ids": [], "confidence": 0.98, "reason": "Same topic"}'

    with patch.object(research_dedup.llm_client, "generate_llm_response", return_value=llm_response):
...
def test_run_dedup_llm_related():
    ...
    llm_response = '{"decision": "related", "target_theme_id": null, "related_ids": ["' + t1["theme_id"] + '"], "confidence": 0.7, "reason": "Related topic"}'
...
def test_run_dedup_llm_failure():
    ...
    with patch.object(research_dedup.llm_client, "generate_llm_response", side_effect=RuntimeError("LLM down")):
        result = research_dedup.run_dedup_review("新テーマ", similar=similar)
...
def test_run_dedup_invalid_json_response():
    ...
    with patch.object(research_dedup.llm_client, "generate_llm_response", return_value="not json at all"):
        result = research_dedup.run_dedup_review("新テーマ", similar=similar)

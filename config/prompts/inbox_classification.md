...
ルール:
- 基準日（今日）は `${today}` です。この日付を基準に「今日」「明日」「来週」「今月20日」などの相対表現を解決し、モデルの内部知識や別の日付を使わない
- 日時は相対表現を基準日から解決し、`YYYY-MM-DDTHH:MM:SS` 形式で指定する
...
- 期限が読み取れる場合は、相対表現を基準日から解決して `due_date` として指定する
- 期限の時刻が明示されない場合は `YYYY-MM-DD`（日付のみ）形式で指定する
- 時刻が明示される場合（「0時」「18時まで」など）のみ `YYYY-MM-DDTHH:MM:SS` 形式で指定する
